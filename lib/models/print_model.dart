...
final Set<String> _printReceiptInProgress = {};
...
Future<void> printReceiptOnceForStatus(pedido, String status) async {
  if (pedido == null) return;

  final orderId = pedido.id?.toString() ?? '';
  if (orderId.isEmpty) return;

  final key = 'printed_${status}_$orderId';
  if (_printReceiptInProgress.contains(key)) return;

  final prefs = await SharedPreferences.getInstance();
  if (prefs.getBool(key) ?? false) return;

  _printReceiptInProgress.add(key);
  await prefs.setBool(key, true);

  try {
    await printReceipt(pedido);
  } catch (_) {
    await prefs.remove(key);
    rethrow;
  } finally {
    _printReceiptInProgress.remove(key);
  }
}
